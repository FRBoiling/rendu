...
namespace rendu {
    ...
    time::Timestamp g_startTime = time::Timestamp::now();
...
string ProcessInfo::procStatus() {
    string result;
    util::readFile("/proc/self/status", 65536, &result);
    return result;
}

string ProcessInfo::procStat() {
    string result;
    util::readFile("/proc/self/stat", 65536, &result);
    return result;
}

string ProcessInfo::threadStat() {
    char buf[64];
    snprintf(buf, sizeof buf, "/proc/self/task/%d/stat", thread::CurrentThread::tid());
    string result;
    util:: readFile(buf, 65536, &result);
    return result;
}
